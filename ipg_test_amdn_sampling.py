...
class MdnAgent():
    ...
    def build_model(self, neurons, components):
        ...
        model = tf.keras.models.Model(inputs=[inputs, labels], outputs=[action])
    ...
    for i in range(1, 121):
        ...
        while pythonapi.sim_isrunning() != 0:       # check if simulation is running
            ...
            if t > 0:
                ...
                with sess.as_default():
                    y = model.predict(x=[np.reshape(inputs, (1, 3)), np.reshape(np.array(0.0), (1,1))], batch_size=1)
        ...
        if not os.path.exists(args.log_dir + '/results_sample_test'):
            os.makedirs(args.log_dir + '/results_sample_test')
        headers = ['t', 'j', 'v', 'a', 'v_lead', 'a_lead', 'x_rel', 'v_rel', 'th', 'y_0', 'y_sc', 'sc']
        with open(args.log_dir + '/results_sample_test/' + str(i) + '.csv', 'w', newline='\n') as f:
