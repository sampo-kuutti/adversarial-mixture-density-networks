...
DATA_DIR = '/vol/research/safeav/Supervised Learning/data_coll/'  # directory data sets are stored in
...
class DataReader(object):
    ...
    """
    def __init__(self, data_dir=DATA_DIR, file_name='dataset_collisions.csv', shuffle=SHUFFLE):
    ...
    def load(self):
        ...
        with open(self.data_dir + self.file_name) as f:
            ...
            for row in reader:
                xs.append(row['v_rel'])  # rel velocity
                xs.append(float(row['th']))  # time headway
                #xs.append(row['Car.ax'])  # host long acc
                xs.append(row['v'])  # host vel
                ys.append(row['y_0'])  # gas - brake pedal value
                total += 1
